...
import { allRecipes, dietPlans, type Recipe } from '../recipes/recipes-data'
...
export default function RecipesStaticPage() {
  ...
  // Filter recipes based on search and diet selection
  const filteredRecipes = useMemo(() => {
    ...
    if (selectedDiet) {
      recipes = recipes.filter(recipe =>
        recipe.diet_plan_names.some(diet => diet.slug === selectedDiet)
      )
    }
    ...
    return recipes.slice(0, displayCount)
  }, [searchQuery, selectedDiet, displayCount])
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          >
            ...
            <p className="text-xl text-white font-medium ml-16 drop-shadow">
              Discover {allRecipes.length} delicious recipes from {dietPlans.length} diet plans
            </p>
          </motion.div>
        </div>
      </header>
      ...
      <section className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8 bg-white rounded-2xl shadow-lg p-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            >
              <option value="">All Diet Plans ({allRecipes.length} recipes)</option>
              {dietPlans.map((plan) => (
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {dietPlans.map((plan) => {
              const count = allRecipes.filter(r =>
                r.diet_plan_names.some(d => d.slug === plan.slug)
              ).length
              return (
                <button
                  key={plan.slug}
                  onClick={() => setSelectedDiet(selectedDiet === plan.slug ? '' : plan.slug)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                    selectedDiet === plan.slug
        ...
        {/* Load More Button */}
        {filteredRecipes.length < allRecipes.length && displayCount < allRecipes.length && (
          <div className="mt-12 text-center">
            <button
              onClick={() => setDisplayCount(prev => prev + 15)}
              className="px-8 py-3 bg-teal-600 text-white font-semibold rounded-xl hover:bg-teal-700 transition transform hover:scale-105"
            >
              Load More Recipes
            </button>
          </div>
